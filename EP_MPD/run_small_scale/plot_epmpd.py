...
dsize = [5, 7, 10, 13]
if shutil.which("latex"):
    dsize_labels = ["$2^{5}$", "$2^{7}$", "$2^{10}$", "$2^{13}$"]
else:
    dsize_labels = ["2^5", "2^7", "2^10", "2^13"]
...
def plot_data(data, data_labels, name):
    ...
    for size in data:
        if name == "dsize":
            ...
            path_t1 = f"type1_runs/10_{size}_0.3.log"
            path_t2 = f"type2_runs/10_{size}_0.3.log"
        elif name == "dup":
            # path_t1 = f"type1_runs/15_10_{size}.log"
            # path_t2 = f"type2_runs/15_10_{size}.log"
            path_t1 = f"type1_runs/10_10_{size}.log"
            path_t2 = f"type2_runs/10_10_{size}.log"
