...
dsize = [10, 11, 13, 15]
if shutil.which("latex"):
    dsize_labels = ["$2^{10}$", "$2^{11}$", "$2^{13}$", "$2^{15}$"]
else:
    dsize_labels = ["2^10", "2^11", "2^13", "2^15"]
...
def plot_data(data, data_labels, name):
    ...
    for size in data:
        if name == "dsize":
            path_t1 = f"local_type1_runs/{size}_50_0.3.log"
            path_t2 = f"local_type2_runs/{size}_50_0.3.log"
        elif name == "clients":
            path_t1 = f"local_type1_runs/19_{size}_0.3.log"
            path_t2 = f"local_type2_runs/19_{size}_0.3.log"
        elif name == "dup":
            path_t1 = f"local_type1_runs/19_50_{size}.log"
            path_t2 = f"local_type2_runs/19_50_{size}.log"
